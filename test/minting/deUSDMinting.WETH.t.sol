...
contract deUSDMintingWETHTest is deUSDMintingUtils {
    ...
    function setUp() public override {
        ...
        vm.prank(owner);
        deUSDMintingContract.addSupportedAsset(address(weth));
    }
    ...
    function testMint() public {
        ...
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), 0);
        ...
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), _stETHToDeposit);
        assertEq(address(deUSDMintingContract).balance, 0);
        assertEq(deUSDToken.balanceOf(beneficiary), _deUSDToMint);
    }

    function testMintWETHWithoutSupport() public {
        vm.prank(owner);
        deUSDMintingContract.removeSupportedAsset(address(weth));
        uint256 amount = _stETHToDeposit;
        assertEq(IERC20(address(weth)).balanceOf(benefactor), amount);
        vm.startPrank(benefactor);
        assertEq(IERC20(address(weth)).balanceOf(benefactor), amount);
        weth.approve(address(deUSDMintingContract), amount);
        assertEq(IERC20(address(weth)).allowance(benefactor, address(deUSDMintingContract)), amount);
        vm.stopPrank();
        (
            IdeUSDMinting.Order memory order,
            IdeUSDMinting.Signature memory takerSignature,
            IdeUSDMinting.Route memory route
        ) = mint_setup(_deUSDToMint, amount, IERC20(address(weth)), 1, false);

        vm.expectRevert(UnsupportedAsset);
        vm.prank(minter);
        deUSDMintingContract.mintWETH(order, route, takerSignature);
        assertEq(IERC20(address(weth)).balanceOf(benefactor), amount);
        // converted to ETH
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), 0);
        assertEq(address(deUSDMintingContract).balance, 0);
    ...
    function testFuzzMintWithWETH(uint256 amount) public {
        ...
        // converted to ETH
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), 0);
        assertEq(address(deUSDMintingContract).balance, amount);
        assertEq(deUSDToken.balanceOf(beneficiary), _deUSDToMint);
    }

    function test_redeem() public {
        executeRedeem(IERC20(address(weth)));
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), 0, "Mismatch in stETH balance");
    ...
    function testRedeemETH() public {
        ...
        bytes32 digest = deUSDMintingContract.hashOrder(redeemOrder);
        takerSignature = signOrder(beneficiaryPrivateKey, digest, IdeUSDMinting.SignatureType.EIP712);
        vm.stopPrank();
        ...
        deUSDMintingContract.redeem(redeemOrder, takerSignature);
        vm.stopPrank();
    ...
    function test_redeem_invalidNonce_revert() public {
        ...
        (IdeUSDMinting.Order memory redeemOrder, IdeUSDMinting.Signature memory takerSignature2) =
            redeem_setup(_deUSDToMint, _stETHToDeposit, IERC20(address(weth)), 1, false);
    ...
    function test_nativeEth_withdraw() public {
        ...
        // taker
        vm.startPrank(benefactor);
        IERC20(address(weth)).approve(address(deUSDMintingContract), _stETHToDeposit);
    ...
    function test_multipleValid_custodyRatios_addresses() public {
        ...
        assertEq(IERC20(address(weth)).balanceOf(address(deUSDMintingContract)), (_stETHToDeposit * 3) / 10);
    ...
    function test_unsupported_assets_ERC20_revert() public {
        vm.startPrank(owner);
        deUSDMintingContract.removeSupportedAsset(address(IERC20(address(weth))));
        vm.stopPrank();
        vm.deal(benefactor, _stETHToDeposit);
        vm.prank(benefactor);
        weth.deposit{value: _stETHToDeposit}();
    ...
    function test_expired_orders_revert() public {
        (
            ...
            IdeUSDMinting.Signature memory takerSignature,
            IdeUSDMinting.Route memory route
        ) = mint_setup(_deUSDToMint, _stETHToDeposit, IERC20(address(weth)), 1, false);
